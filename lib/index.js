...
const addPluginAttributes = (el, binding, options) => {
  ...
  if (typeof binding.value !== 'undefined') {
    ...
    if (binding.value.name) name = binding.value.name;
    else if (src) name = src.split('/')[src.split('/').length - 1].split('?')[0];
    else name = title || 'image';
  ...
  if (name) el.setAttribute('data-vue-img-name', name);
  ...
  Vue.directive('img', {
    ...
    bind(el, binding) {
      ...
      el.addEventListener(addedAttributes.openOn, () => {
        ...
        Vue.set(vm, 'names', images.map(e => e.dataset.vueImgName));
